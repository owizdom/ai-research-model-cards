...
### 3. The corpus is six labs now (was nine)

Earlier snapshots tracked nine labs; Cohere, Amazon, and AI21 have since been dropped and the current DB has six (Anthropic, OpenAI, Google, Meta, Mistral, xAI). Older text that cites "9 labs / 88 docs / 1417 evals" describes a previous snapshot. Current canonical counts live in `data/stats.json`, generated by `scripts/compute_stats.py`: **6 labs / 79 docs / 1,634 evals** as of 2026-05-31. Regenerate stats.json after any corpus change instead of hand-editing counts into the docs.
